...
export function useEvents(options: UseEventsOptions = {}): UseEventsResult {
  const { limit = 20, city, segment, upcoming = true } = options;
  ...
  const fetchEvents = async () => {
    try {
      ...
      // Transform database events to Event type
      const transformedEvents: Event[] = (data.events || []).map((dbEvent: any) => {
        ...
        // Determine icon based on segment/genre
        ...
        return {
          ...
          type: 'event' as const, // All Ticketmaster events are type 'event'
